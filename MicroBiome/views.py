...
from .models import (Amplicon, Assay, BodySite, Disease, LocEthDiet, Platform,
                     Project, Samples, TestProject)
...
def loc_eth_diest_update(df):
    for _, row in df.drop_duplicates(
            ["COUNTRY", "REGION", "URBANZATION", "CITYVILLAGE",
                "ETHNICITY", "DIET", "LAT LON"]).iterrows():
        print(row)
...
def bodysite_update(df):
    for _, row in df.drop_duplicates(
            ["BODY SITE"]).iterrows():
        BodySite.objects.get_or_create(
        ...
        if not Samples.objects.filter(sampid__exact=row["Run ID"]):
            ...
            samp = Samples.objects.create(
                ...
                bodysite=BodySite.objects.filter(
                    bodysite__exact=bodysite['BODY SITE'])[0],
...
def search_form(request):
    ...
    # print(dft.head())
    body_site_project = BodySite.objects.all().annotate(num_samples=Count("samples__project", distinct=True))
    body_site_sample = BodySite.objects.all().annotate(num_samples=Count("samples"))
    body_site_pie_project = [{'name': plts.bodysite,
                          'y': plts.num_samples} for plts in body_site_project
                         if plts.bodysite != "nan"]
    body_site_pie_sample = [{'name': plts.bodysite,
                          'y': plts.num_samples} for plts in body_site_sample
                         if plts.bodysite != "nan"]

    assay_project = Assay.objects.all().annotate(num_samples=Count("samples__project", distinct=True))
    assay_sample = Assay.objects.all().annotate(num_samples=Count("samples"))
    assay_pie_project = [{'name': plts.assay,
                          'y': plts.num_samples} for plts in assay_project
                         if plts.assay != "nan"]
    assay_pie_sample = [{'name': plts.assay,
                          'y': plts.num_samples} for plts in assay_sample
                         if plts.assay != "nan"]
...
#                           if not (bs.bodysite in ["Ebola virus",'nan', 'penil,vaginal'] or 'metagenom' in bs.bodysite)]
    ...
    disease_project = Disease.objects.all().annotate(num_samples=Count("samples__project", distinct=True))
    disease_sample = Disease.objects.all().annotate(num_samples=Count("samples"))
    disease_pie_project = [{'name': plts.disease,
                          'y': plts.num_samples} for plts in disease_project
                         if plts.disease != "nan"]
    disease_pie_sample = [{'name': plts.disease,
                          'y': plts.num_samples} for plts in disease_sample
                         if plts.disease != "nan"]
    ...
    geoloc = pd.DataFrame(LocEthDiet.objects.values(
        "lon", "lat","country").annotate(num_project=Count("samples__project", distinct=True)))
    geoloc_samp = pd.DataFrame(LocEthDiet.objects.values("country").annotate(num_samples=Count("samples")))
    ...
    geoloc = geoloc.groupby(["lon", "lat","country"])["num_project"].apply(sum).reset_index()
...
def results_sample(request):
    ...
    if request.method == "GET":
        ...
        res = Samples.objects.filter(query).values("sampid", "locetdiet__country", "platform__platform",
            "amplicon__amplicon", "assay__assay", "disease__disease", "locetdiet__lon", "locetdiet__lat")
        df = read_frame(res).replace("nan", "")

        print(df)
        paginator = Paginator(df.to_dict(orient="records"), 10) # 10 information per page
        ...
        page_range = paginator.page_range[start_index:end_index]
         # Platforms
        platform = read_frame(res.annotate(num=Count("platform__platform")).order_by("platform__platform"))
        platform = platform.groupby("platform__platform")["num"].apply(np.sum).reset_index()

        platform_pie_dict =[{'name': plts["platform__platform"],
                            'y': plts["num"]} for _, plts in platform.iterrows()
                            if plts["platform__platform"] != "nan"]



        # ASSAY
        assay = read_frame(res.annotate(num=Count("assay__assay")).order_by("assay__assay"))
        assay = assay.groupby("assay__assay")["num"].apply(np.sum).reset_index()
        print(assay)

        assay_pie_dict =[{'name': plts["assay__assay"],
                              'y': plts["num"]} for _, plts in assay.iterrows()
                             if plts["assay__assay"] != "nan"]


        # DISEASES
        disease = read_frame(res.annotate(num=Count("disease__disease")).order_by("disease__disease"))
        disease = disease.groupby("disease__disease")["num"].apply(np.sum).reset_index()

        disease_pie_dict =[{'name': plts["disease__disease"],
                              'y': plts["num"]} for _, plts in disease.iterrows()
        ...
        try:
            ...
            geoloc = geoloc.groupby(["locetdiet__lon", "locetdiet__lat","locetdiet__country"]).size().reset_index().rename(columns={0:"num_samples","locetdiet__lon":"lon", "locetdiet__lat":"lat","locetdiet__country":"country" })
        ...
        return render(request,"sample_results.html",{'results': items, # "results_samples.html"
                                                'tags': tags,
                                                'extras':extras,
                                                'platform_pie_dict_sample': platform_pie_dict if platform_pie_dict else no_data,
                                                'assay_pie_dict_sample': assay_pie_dict if assay_pie_dict else no_data,
                                                'disease_pie_dict_sample': disease_pie_dict if disease_pie_dict else no_data,
                                                'records': geoloc,
                                                # Pagination
...
def results(request):
    ...
    if not tags:
        ...
        res = Samples.objects.filter().values("project__repoid", "project__title", "locetdiet__country", "platform__platform",
        "amplicon__amplicon", "assay__assay", "disease__disease", "locetdiet__lon", "locetdiet__lat"
        )
    else:
        ...
        res = Samples.objects.filter(query
        ).values("project__repoid", "project__title", "locetdiet__country", "platform__platform",
        "amplicon__amplicon", "assay__assay", "disease__disease", "locetdiet__lon", "locetdiet__lat","locetdiet__cityvillage", "locetdiet__urbanization","locetdiet__diets", "locetdiet__ethnicity", "locetdiet__region"
    ...
    try:
        df = read_frame(res.values('project__repoid', "project__title" ).annotate(samp_size=Count("project__repoid")).order_by('project__repoid'),
        fieldnames=['project__repoid', "project__title","samp_size"])
        # print(df)
        df2 = read_frame(res).melt(id_vars=["project__repoid"],
        value_vars=["locetdiet__country","locetdiet__cityvillage", "locetdiet__urbanization","locetdiet__diets", "locetdiet__ethnicity", "locetdiet__region", "platform__platform",
            "amplicon__amplicon", "assay__assay","disease__disease"]).groupby(["project__repoid",  "variable", "value"]).size().reset_index(
            ).sort_values(0, ascending=False)#.drop_duplicates(["project__repoid", "variable"])
        ...
        df2 = df2.groupby(["project__repoid", "variable"])["value"].apply(add_sub).reset_index()
        # del df2[0]
        # df2.to_csv("anmol.csv", index=False)
        df2 = df2.pivot(index="project__repoid",
...
# # DISEASES
    ...
    if qt == "get":
        return render(request, "results.html", {'results': items,
                                            'tags': tags,
                                            "qt":qt,
        ...
        return render(None, "results_refine.html", {'results': items,
                                            # 'tags': tags,
                                            # "qt":qt,
                                            'platform_pie_dict': platform_pie_dict,
                                            'assay_pie_dict': assay_pie_dict,
